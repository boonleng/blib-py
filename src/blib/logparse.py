...
from blib.cosmetics import colorize
from blib.network import get_user_agent_string, get_ip_location
from blib.__init__ import __version__
...
class LogParser:
    ...
    def decode(self, line):
        line = line.rstrip()
        x = self.parser.match(line)
        if x:
            x = x.groupdict()
            self.ip = x["ip"]
            self.datetime = fast_parse_time(x["time"])
